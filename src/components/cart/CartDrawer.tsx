    ...
    <AnimatePresence>
      {isDrawerOpen && (
        <>
          ...
          {/* Drawer */}
          <motion.aside
            className="fixed top-0 right-0 bottom-0 w-full max-w-[460px] bg-white z-[61] flex flex-col shadow-2xl"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-mk-line">
              <div className="flex items-center gap-2">
                <ShoppingCart size={20} className="text-mk-navy" />
                <span className="text-lg font-bold text-mk-navy">Mon Panier</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-sm text-mk-sec">{cartCount} article{cartCount > 1 ? "s" : ""}</span>
                <button onClick={closeDrawer} className="text-mk-sec hover:text-mk-navy transition-colors">
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              {items.length === 0 ? (
                <div className="text-center py-12">
                  <ShoppingCart className="mx-auto text-mk-sec mb-3" size={40} />
                  <p className="text-mk-sec text-sm">Votre panier est vide</p>
                </div>
              ) : (
                <div className="space-y-0">
                  ...
                  <div className="bg-mk-alt rounded-t-lg px-4 py-3 border border-mk-line">
                    <div className="flex items-center justify-between">
                      <h3 className="font-semibold text-mk-navy text-[15px]">
                  ...
                  <div className="border-x border-mk-line">
                    {items.map((item, i) => (
                      <motion.div
                        key={item.id}
                        className="px-4 py-3 border-b border-mk-line"
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-mk-navy leading-tight">
                              {item.product?.name || "Produit"}
                            </p>
                            <p className="text-xs text-mk-ter mt-0.5">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <div className="flex items-center border border-mk-line rounded-md bg-white">
                              <button
                                ...
                                className="px-1.5 py-1 text-mk-sec hover:text-mk-navy"
                              >
                                <Minus size={12} />
                              </button>
                  ...
                  <div className="border border-mk-line rounded-b-lg px-4 py-3 bg-white">
                    <div className="flex items-center gap-2 mb-2">
                      <Package size={14} className="text-mk-navy" />
                      <span className="text-sm font-medium text-mk-navy">Franco de port</span>
                    </div>
                    <div className="flex justify-between text-sm mb-2">
                      <span className="text-mk-sec">Sous-total vendeur:</span>
                      <span className="font-semibold text-mk-navy">{formatPrice(total)}€</span>
                    </div>
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden mb-2">
                      <motion.div
                        className="h-full rounded-full"
                        style={{ backgroundColor: francoProgress >= 100 ? "#16A34A" : "#F97316" }}
                        initial={{ width: 0 }}
                        animate={{ width: `${francoProgress}%` }}
                        transition={{ duration: 0.6, ease: "easeOut" }}
                      />
                    </div>
                    {francoProgress >= 100 ? (
                      <p className="text-sm text-mk-green font-medium flex items-center gap-1">
                        ✓ Livraison gratuite pour ce vendeur
                      </p>
                    ) : (
                      <p className="text-sm text-orange-500">
                        Plus que {formatPrice(francoRemaining)}€ pour la livraison gratuite
                      </p>
                    )}
                  </div>
                  ...
                  <div className="mt-4 space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-mk-sec">Sous-total produits HT:</span>
                      <span className="font-semibold text-mk-navy">{formatPrice(total)}€</span>
                    </div>
                  </div>
                </div>
            ...
            {items.length > 0 && (
              <div className="border-t border-mk-line px-5 py-4 space-y-3">
                <div className="flex items-center justify-between text-xs text-mk-ter">
                  <div className="flex items-center gap-1">
                    <Package size={12} />
                    <span>Franco de port</span>
                  </div>
                  <span>{formatPrice(Math.min(total, francoTarget))}€ / {francoTarget}€</span>
                </div>
                {francoRemaining > 0 && (
