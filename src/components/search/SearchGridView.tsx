...
function SearchGridCard({ p, onClick }: CardProps) {
  const price = p.price;
  const pub = p.pub;
  const pct = p.pct;
  ...
  return (
    ...
    >
      ...
      <div className="p-3 space-y-1.5">
        ...
        <p className="text-sm font-semibold text-foreground line-clamp-2 leading-tight">{displayName}</p>
        {displayShortDescription && (
          <p className="text-[11px] text-muted-foreground line-clamp-2 leading-snug">{displayShortDescription}</p>
        )}
        <div className="flex items-baseline gap-2">
          <span className="text-base font-bold text-foreground">{price.toFixed(2)} €</span>
          {pub > price && (
            <span className="text-xs text-muted-foreground line-through">{pub.toFixed(2)} €</span>
          )}
        </div>
...
export default function SearchGridView({ products }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {products.map((p) => (
        <SearchGridCard key={p.id} p={p} onClick={() => navigate(`/produit/${p.slug}`, fromState)} />
      ))}
    </div>
  );
}
