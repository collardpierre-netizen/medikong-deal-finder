...
export function useCatalogProducts(filters: CatalogFilters) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        .from("products")
        .select("id, slug, name, brand_name, brand_id, category_id, category_name, gtin, cnk_code, image_urls, short_description, is_promotion, promotion_label, best_price_excl_vat, best_price_incl_vat, offer_count, total_stock, is_in_stock, created_at, brands(slug)", { count: hasFilters ? "exact" : "estimated" })
        .eq("is_active", true);
