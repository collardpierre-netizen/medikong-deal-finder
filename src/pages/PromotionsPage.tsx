...
export default function PromotionsPage() {
  ...
  return (
    <Layout>
      {/* Hero */}
      <div className="py-10" style={{ background: "linear-gradient(135deg, #1E293B, #1B5BDA)" }}>
        <div className="mk-container text-center">
          <h1 className="text-[32px] font-bold text-white mb-2">Promotions en cours</h1>
          ...
          <div className="flex justify-center gap-3">
            {[
              ...
              <div key={l as string} className="px-5 py-3 rounded-lg text-center" style={{ background: "rgba(255,255,255,0.2)" }}>
      ...
      <div className="mk-container py-8">
        ...
        <div className="flex gap-2 mb-6">
          {filters.map((f, i) => (
            <button key={f} onClick={() => setActiveFilter(i)} className={`px-4 py-1.5 rounded-full text-sm font-medium ${i === activeFilter ? "bg-mk-navy text-white" : "border border-mk-line text-mk-sec"}`}>{f}</button>
          ))}
        </div>
        ...
        {/* Upcoming */}
        <div className="rounded-lg p-6" style={{ background: "#FEF3C7" }}>
