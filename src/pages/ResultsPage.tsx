    ...
    <Layout>
      ...
      <div className="mk-container py-6">
        ...
        <div className="flex items-center gap-2 mb-4">
          <button className="flex items-center gap-1.5 px-3 py-1.5 border border-mk-line rounded-md text-sm text-mk-sec">
          ...
          {filterPills.map((p, i) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-sm font-medium ${i === activePill ? "bg-mk-navy text-white" : "border border-mk-line text-mk-sec"}`}
            >
              {p}
            </button>
          ))}
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <div className="border border-mk-line rounded-lg overflow-hidden">
                <div className="grid grid-cols-[60px_1fr_80px_100px_80px_80px_100px] gap-3 px-4 py-2 bg-mk-alt text-xs font-semibold text-mk-sec">
                  <span></span><span>Produit</span><span>CNK</span><span>Marque</span><span>Prix</span><span>Offres</span><span>Action</span>
                </div>
                {products.map(p => (
                  <Link key={p.id} to={`/produit/${p.slug}`} className="grid grid-cols-[60px_1fr_80px_100px_80px_80px_100px] gap-3 px-4 py-3 items-center border-t border-mk-line hover:bg-mk-alt text-sm">
              ...
              <div className="space-y-4">
                <div className="flex gap-3 mb-4">
                  {[
                    ...
                    <button key={t.label} className={`px-4 py-2 rounded-md text-sm font-medium ${t.active ? "bg-mk-navy text-white" : "border border-mk-line text-mk-sec"}`}>
                      {t.label} · {t.price} {t.sub && <span className="text-xs opacity-70">({t.sub})</span>}
                    </button>
                  ))}
                </div>
                {products.map((p, i) => (
                  <div key={p.id} className="border border-mk-line rounded-lg flex overflow-hidden animate-fadeInUp" style={{ animationDelay: `${i * 70}ms` }}>
                    <div className="w-[200px] shrink-0 relative p-4">
                    ...
                    <div className="flex-1 p-4 border-l border-mk-line">
                    ...
                    <div className={`w-[260px] shrink-0 p-4 border-l border-mk-line ${p.mk ? "bg-mk-deal" : ""}`}>
