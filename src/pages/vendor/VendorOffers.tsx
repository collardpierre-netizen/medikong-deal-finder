...
import { OfferCommissionOverrideDialog } from "@/components/vendor/OfferCommissionOverrideDialog";
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [{ data: offers }, { data: marketPrices }, { data: externalOffers }] = await Promise.all([
        ...
        (supabase.rpc as any)("get_market_prices_for_products", { _product_ids: [productId] })
          .then((r: any) => ({ ...r, data: (r.data || []).slice(0, 10).map((m: any) => ({ ...m, market_price_sources: { name: m.source_name, source_type: m.source_type } })) })),
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {offers.length > 0 && (
            <button onClick={() => exportOffers(offers, profileRulesMap, priceTiersMap, commissionConfig)}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-[12px] font-medium border transition-colors hover:bg-[#F8FAFC]"
              style={{ borderColor: "#E2E8F0", color: "#616B7C" }}>
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {offer.product_id && vendor?.id && (
                            <VendorCommissionOverrideDialog
                              ...
                              offerId={offer.id}
                              trigger={
                                <button className="p-1.5 hover:bg-[#FEF3C7] rounded" title="Commission par produit (s'applique à toutes vos offres sur ce produit)">
                                  <Percent size={14} style={{ color: "#D97706" }} />
                                </button>
                              }
                            />
                          )}
                          <OfferCommissionOverrideDialog
                            offerId={offer.id}
                            productName={prod?.name}
                            trigger={
                              <button
                                className="p-1.5 hover:bg-[#FFF7ED] rounded relative"
                                title="Override de commission sur CETTE offre uniquement (priorité maximale)"
                              >
                                <Tag size={14} style={{ color: "#EA580C" }} />
                                {offer.commission_model && (
                                  <span
                                    className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full"
                                    style={{ backgroundColor: offer.commission_override_status === "approved" ? "#059669" : "#F59E0B" }}
                                    aria-label={`Override ${offer.commission_override_status}`}
                                  />
                                )}
                              </button>
                            }
