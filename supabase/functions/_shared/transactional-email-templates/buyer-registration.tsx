...
const BuyerRegistrationEmail = ({
  ...
  <Html lang="fr" dir="ltr">
    <Head />
    <Preview>🆕 Nouvel acheteur inscrit : {companyName}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Img src={LOGO_URL} width="180" alt="MediKong" style={{ marginBottom: '24px' }} />
        <Heading style={h1}>Nouvel acheteur en attente</Heading>
        <Text style={text}>
          Un nouvel acheteur vient de s'inscrire sur {SITE_NAME} et attend votre validation.
        </Text>
        <Section style={summaryBox}>
          ...
          <Row><Column style={label}>N° TVA</Column><Column style={value}>{vatNumber || '—'}</Column></Row>
          <Row><Column style={label}>Secteur</Column><Column style={value}>{sector || '—'}</Column></Row>
          <Row><Column style={label}>Pays</Column><Column style={value}>{country}</Column></Row>
        </Section>
        <Button href="https://medikong.pro/admin/users" style={button}>
...
export const template = {
  component: BuyerRegistrationEmail,
  subject: (data: Record<string, any>) => `🆕 Nouvel acheteur : ${data.companyName || 'Nouveau compte'}`,
  displayName: 'Inscription acheteur (admin)',
  previewData: { companyName: 'PharmaCorp SPRL', email: 'info@pharmacorp.be', phone: '+32 2 123 45 67', sector: 'Pharmacie', country: 'Belgique', vatNumber: 'BE0746993139' },
