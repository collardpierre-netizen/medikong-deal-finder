...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: lines, error: lErr } = await admin
      .from("order_lines")
      .select("id, offer_id, quantity, unit_price_excl_vat, vat_rate, line_total_excl_vat, cost_price, manual_label, vendor_reference, tracking_number, tracking_url, status, products(name, gtin, cnk_code), offers(vendor_reference)")
      .eq("order_id", orderId)
