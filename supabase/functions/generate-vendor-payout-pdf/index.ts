...
// Filters to the calling vendor's own lines on a given order.
// Optional `tracking_number` restricts the statement to a single shipment.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let linesQuery = admin
      .from("order_lines")
      .select("id, offer_id, quantity, quantity_shipped, unit_price_excl_vat, vat_rate, line_total_excl_vat, manual_label, cnk_code, tracking_number, tracking_url, tracking_carrier, fulfillment_status, cost_price, products(name, gtin, cnk_code)")
      .eq("order_id", orderId)
    ...
    if (!lines || lines.length === 0) return json(404, { error: "no_lines_found" });
    ...
    if (trackingFilter) {
      ...
      doc.setTextColor(...NAVY);
      doc.text(`Expédition ${trackingFilter}`, M + 18, y + 27);
    } else {
      doc.setTextColor(...MUTED);
      doc.text("Portée", M, y + 27);
      doc.setTextColor(...NAVY);
      doc.text("Toutes les lignes vendeur", M + 18, y + 27);
    }
    ...
    doc.setFontSize(7.5);
    let rowIdx = 0;
    for (const { l, cfg, lineHt, lineCommission } of computedLines) {
