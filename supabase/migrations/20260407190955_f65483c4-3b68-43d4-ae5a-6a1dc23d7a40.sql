...
BEGIN
  ...
  LOOP
    IF r.gap_pct >= _th_crit THEN v_severity := 'critical'::alert_severity;
    ELSIF r.gap_pct >= _th_warn THEN v_severity := 'warning'::alert_severity;
    ELSE v_severity := 'info'::alert_severity;
    END IF;

    v_alert_type := 'market_price'::alert_type;

    SELECT id INTO v_existing_id FROM price_alerts
    WHERE product_id = r.product_id AND status IN ('new'::alert_status, 'seen'::alert_status, 'in_progress'::alert_status)
    LIMIT 1;
    ...
    ELSE
      ...
      VALUES (r.product_id, v_alert_type, v_severity, r.ref_price, r.mk_price, r.gap_amt, r.gap_pct, 'new'::alert_status);
  ...
  SET status = 'auto_resolved'::alert_status, resolved_at = now()
  FROM products p
  JOIN competitor c ON c.product_id = p.id
  WHERE pa.product_id = p.id
    AND pa.status IN ('new'::alert_status, 'seen'::alert_status, 'in_progress'::alert_status)
    AND p.best_price_incl_vat <= c.best_price;
