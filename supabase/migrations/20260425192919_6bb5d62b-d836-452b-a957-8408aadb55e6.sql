...
CREATE OR REPLACE FUNCTION public.get_vendor_market_intelligence(_vendor_id uuid)
 RETURNS TABLE(
   product_id uuid, product_name text, product_image text, gtin text, cnk_code text, brand_name text,
   country_code text,
   product_discount_percentage numeric,
   my_offer_id uuid, my_price_excl_vat numeric, my_stock integer,
   my_stock_status text, my_updated_at timestamptz,
   my_rank integer,
   medikong_competitors_count integer, medikong_total_offers integer,
   best_medikong_competitor_price numeric, best_medikong_competitor_vendor text,
   medikong_median_price numeric,
   gap_vs_best_amount numeric, gap_vs_best_percentage numeric,
   gap_vs_median_amount numeric, gap_vs_median_percentage numeric,
   external_sources_count integer, best_external_price numeric, best_external_source text,
   competitors_in_stock integer, competitors_on_promo integer,
   medikong_offers jsonb, external_offers jsonb
 )
 LANGUAGE sql
...
AS $function$
  WITH my_offers AS (
    SELECT o.id AS offer_id, o.product_id, o.country_code, o.price_excl_vat, o.stock_quantity,
           o.stock_status::text AS stock_status, o.updated_at
    FROM offers o
  ...
  mk_offers_agg AS (
    SELECT
      ...
      m.stock_status AS my_stock_status,
      m.updated_at AS my_updated_at,
      COUNT(*) FILTER (WHERE o.vendor_id <> _vendor_id) AS competitors_count,
      COUNT(*) AS total_offers,
      COUNT(*) FILTER (WHERE o.vendor_id <> _vendor_id AND COALESCE(o.stock_quantity, 0) > 0) AS competitors_in_stock,
      COUNT(*) FILTER (WHERE o.vendor_id <> _vendor_id AND COALESCE(p2.discount_percentage, 0) > 0) AS competitors_on_promo,
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'is_promo', (COALESCE(p2.discount_percentage, 0) > 0),
          'promo_discount', p2.discount_percentage,
    ...
    GROUP BY m.product_id, m.country_code, m.offer_id, m.price_excl_vat, m.stock_quantity, m.stock_status, m.updated_at
  ),
  ...
  SELECT
    ...
    a.competitors_in_stock::integer,
    a.competitors_on_promo::integer,
    a.medikong_offers,
