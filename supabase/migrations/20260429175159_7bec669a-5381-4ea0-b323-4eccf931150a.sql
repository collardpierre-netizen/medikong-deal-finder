-- 1. Ajout aliases + norm_key sur manufacturers
ALTER TABLE public.manufacturers
  ADD COLUMN IF NOT EXISTS aliases text[] DEFAULT '{}'::text[];

-- norm_key calculé via la même fonction que les brands
ALTER TABLE public.manufacturers
  ADD COLUMN IF NOT EXISTS norm_key text
    GENERATED ALWAYS AS (public.normalize_brand_name(name)) STORED;

CREATE INDEX IF NOT EXISTS idx_manufacturers_norm_key ON public.manufacturers (norm_key);
CREATE INDEX IF NOT EXISTS idx_manufacturers_aliases_gin ON public.manufacturers USING GIN (aliases);
...
BEGIN
  ...
  SELECT
    CASE WHEN _dry_run THEN (SELECT COUNT(*) FROM candidates)
         ELSE (SELECT COUNT(*) FROM upd) END
  INTO v_via_brand;

  -- ============== Pass 2: market_prices.supplier_name → manufacturer ==============
  WITH candidates AS (
    ...
    WHERE mp.is_matched = true
      AND mp.supplier_name IS NOT NULL
      AND mp.supplier_name <> ''
  ...
  -- ============== Pass 3: dictionnaire brand_name → manufacturer ==============
  -- Match si la marque (texte brut ou résolue) correspond à un fabricant connu.
  WITH candidates AS (
    SELECT DISTINCT ON (p.id) p.id AS product_id, m.id AS new_mfr
    FROM public.products p
    LEFT JOIN public.brands b ON b.id = p.brand_id
    JOIN public.manufacturers m
      ON m.norm_key = COALESCE(
           b.norm_key,
           public.normalize_brand_name(p.brand_name)
         )
      OR COALESCE(
           b.norm_key,
           public.normalize_brand_name(p.brand_name)
  ...
  -- Refresh manufacturer counts (skip on dry-run)
  ...
  SET status = 'success'::sync_log_status,
      completed_at = now(),
...
REVOKE ALL ON FUNCTION public.resolve_product_manufacturers(boolean,integer) FROM public;
GRANT EXECUTE ON FUNCTION public.resolve_product_manufacturers(boolean,integer) TO authenticated;