...
-- 1) Table vendor_order_tokens : 1 token par sub_order, TTL absolu 90j
CREATE TABLE IF NOT EXISTS public.vendor_order_tokens (
  sub_order_id uuid PRIMARY KEY REFERENCES public.sub_orders(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_vendor_order_tokens_token ON public.vendor_order_tokens(token);
CREATE INDEX IF NOT EXISTS idx_vendor_order_tokens_vendor ON public.vendor_order_tokens(vendor_id);
CREATE INDEX IF NOT EXISTS idx_vendor_order_tokens_order ON public.vendor_order_tokens(order_id);

ALTER TABLE public.vendor_order_tokens ENABLE ROW LEVEL SECURITY;
-- Aucune policy : RLS active + 0 policy = deny all sauf service_role.

-- 2) Patch fanout_order_to_vendors : DROP nécessaire car return type change
DROP FUNCTION IF EXISTS public.fanout_order_to_vendors(uuid);

CREATE FUNCTION public.fanout_order_to_vendors(_order_id uuid)
 RETURNS TABLE(
   vendor_id uuid,
   vendor_email text,
   vendor_name text,
