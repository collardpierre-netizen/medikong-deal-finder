...
BEGIN
  ...
  RETURN QUERY
  WITH targets AS (
    SELECT t.vendor_id AS v_id, t.reason AS v_reason
    FROM public.rfq_select_top_vendors(_rfq_id) t
  ),
  inserted AS (
    INSERT INTO public.rfq_dispatch_log AS d (rfq_id, vendor_id, reason, status)
    SELECT _rfq_id, t.v_id, t.v_reason, 'dispatched'
    FROM targets t
    ON CONFLICT (rfq_id, vendor_id) DO NOTHING
    RETURNING d.vendor_id AS v_id, d.reason AS v_reason
  ),
  notifs AS (
    INSERT INTO public.vendor_notifications AS n (vendor_id, type, title, body, cta_url, payload)
    SELECT i.v_id, 'rfq_received', 'Nouvelle demande de prix',
           ...
           jsonb_build_object(
             'rfq_id', _rfq_id, 'reason', i.v_reason::text,
    ...
    FROM inserted i RETURNING n.id AS n_id, n.vendor_id AS v_id
  ),
  link AS (
    UPDATE public.rfq_dispatch_log d
    SET notification_id = n.n_id
    FROM notifs n
    WHERE d.rfq_id = _rfq_id AND d.vendor_id = n.v_id
    RETURNING d.vendor_id AS v_id
  )
  SELECT t.v_id, t.v_reason,
         EXISTS (SELECT 1 FROM inserted i WHERE i.v_id = t.v_id) AS was_new
  FROM targets t;
