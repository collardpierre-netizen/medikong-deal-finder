...
BEGIN
  ...
  IF _scope = 'admin' THEN
    ...
    SELECT count(*) INTO _c_orders FROM public.sub_orders WHERE status = 'pending';
      ...
      FROM (
        ...
        SELECT * FROM (
          SELECT 'order'::text, ('Commande à traiter'),
                 ('Sub-order ' || substr(id::text,1,8)),
                 '/admin/commandes', created_at
          FROM public.sub_orders WHERE status = 'pending'
          ORDER BY created_at DESC LIMIT 5
        ) e
  ...
  ELSIF _scope = 'vendor' THEN
    ...
    SELECT count(*) INTO _c_vorders FROM public.sub_orders
