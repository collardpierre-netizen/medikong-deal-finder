...
BEGIN
  ...
  SET
    fulfillment_status = _status::order_fulfillment_status,
...
GRANT EXECUTE ON FUNCTION public.vendor_update_order_line_status(uuid, text, integer, text, text, text, numeric) TO authenticated;

DROP POLICY IF EXISTS "Vendors read own sub_orders" ON public.sub_orders;

CREATE POLICY "Vendors read own sub_orders"
