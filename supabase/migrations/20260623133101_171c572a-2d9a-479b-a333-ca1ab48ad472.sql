...
BEGIN
  ...
  LOOP
    ...
    -- per-line commission: prefer fixed amount, else rate * line_excl
    v_line_commission := COALESCE(
      CASE WHEN v_line_amount IS NOT NULL THEN round(v_line_amount * v_qty, 2) END,
      CASE WHEN v_line_rate IS NOT NULL THEN round(v_line_excl * v_line_rate / 100.0, 2) END,
      0
    );
    ...
    -- aggregate per-line commissions per vendor (only if any line provided one)
  ...
  LOOP
    ...
    -- vendor-level commission still supported as fallback if no per-line value
    ...
    -- per-line aggregation overrides vendor-level commission
