CREATE OR REPLACE FUNCTION public.admin_check_orders_coherence(_order_ids uuid[] DEFAULT NULL)
RETURNS TABLE (
  order_id uuid,
  order_number text,
  status text,
...
GRANT EXECUTE ON FUNCTION public.admin_check_orders_coherence(uuid[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_check_orders_coherence(uuid[]) TO service_role;