...
-- 1) Add per-line commission persistence columns
ALTER TABLE public.order_lines
  ADD COLUMN IF NOT EXISTS commission_rate numeric,
  ADD COLUMN IF NOT EXISTS commission_amount numeric,
  ADD COLUMN IF NOT EXISTS commission_basis text;

-- 2) Patch admin_create_manual_order to persist these on order_lines
...
BEGIN
  ...
  INSERT INTO public.orders (
    ...
    admin_notes, created_by_user_id, created_at, is_forecast
  ) VALUES (
    ...
    v_notes, auth.uid(),
    COALESCE(v_created_at, now()), v_is_forecast
  ...
  LOOP
    ...
    INSERT INTO public.order_lines (
      ...
      v_line_rate, v_line_amount, v_line_basis
    );
...
-- 3) Patch admin_update_manual_order to persist per-line commission too
...
-- 4) Patch admin_duplicate_order_payload to read per-line commission from order_lines
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    -- Prefer per-line stored values, fall back to vendor-level reconstruction
