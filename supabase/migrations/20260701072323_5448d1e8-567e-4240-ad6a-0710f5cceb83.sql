...
BEGIN
  ...
  SELECT
    ...
    jsonb_build_object(
      'gmv_ht', COALESCE((SELECT SUM(subtotal_excl_vat) FROM filtered), 0),
      'gmv_ttc', COALESCE((SELECT SUM(total_incl_vat) FROM filtered), 0),
