...
-- Fix public_get_order_by_token: it was querying vendor_order_tokens.token which doesn't exist.
-- Rewrite the 2-arg overload to use orders.public_token / public_access_pin / public_access_expires_at.
DROP FUNCTION IF EXISTS public.public_get_order_by_token(text);
DROP FUNCTION IF EXISTS public.public_get_order_by_token(text, text);
...
BEGIN
  ...
  IF v_order.public_access_expires_at IS NOT NULL
     AND v_order.public_access_expires_at < now() THEN
  ...
  IF (v_lines IS NULL OR v_lines = '[]'::jsonb)
    ...
    SELECT coalesce(jsonb_agg(jsonb_build_object(
      ...
      'product_gtin', p.gtin,
      'product_cnk', p.cnk_code,
      'quantity', nullif(l->>'quantity','')::numeric,
      'unit_price_excl_vat', nullif(l->>'unit_price_excl_vat','')::numeric,
      'vat_rate', nullif(l->>'vat_rate','')::numeric,
      'line_total_excl_vat', nullif(l->>'line_total_excl_vat','')::numeric,
      'vendor_name', coalesce(v.company_name, v.name)
    )), '[]'::jsonb)
    INTO v_lines
