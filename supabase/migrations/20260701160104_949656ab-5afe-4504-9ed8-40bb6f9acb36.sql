...
BEGIN
  ...
  -- customers guard
  INSERT INTO public.customers (id, is_verified, credit_limit, is_active, customer_type, payment_terms_days)
  VALUES (_cust_id, false, 0, true, 'pharmacie', 30);
  ...
  BEGIN
    UPDATE public.customers SET is_active = false WHERE id = _cust_id;
    _results := _results || jsonb_build_object('case','customers.is_active blocked','passed',false,'detail','no exception');
  EXCEPTION WHEN OTHERS THEN
    _err := SQLERRM;
    _results := _results || jsonb_build_object('case','customers.is_active blocked','passed', _err ILIKE '%privileged customer fields%','detail', _err);
  END;
  ...
  IF _admin_uid IS NOT NULL THEN
    ...
    INSERT INTO public.customers (id, is_verified, credit_limit, is_active, customer_type)
    VALUES (_cust_id, false, 0, true, 'pharmacie');
...
REVOKE ALL ON FUNCTION public.admin_test_privileged_column_guards() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_test_privileged_column_guards() TO authenticated, service_role;
