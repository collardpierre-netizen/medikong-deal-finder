...
BEGIN
  ...
  -- Trigger if no successful run in the last 7 days (168h) or never
  IF v_last_success IS NULL OR v_hours_since > 168 THEN
    -- De-dup: skip if an unresolved alert was already emitted in the last 24h
...
DO $$
BEGIN
  PERFORM cron.unschedule('qogita-stale-refresh-alert-daily');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

SELECT cron.schedule(
  'qogita-stale-refresh-alert-daily',
  '0 8 * * *',
  $$SELECT public.check_qogita_stale_refresh_alert();$$
);
