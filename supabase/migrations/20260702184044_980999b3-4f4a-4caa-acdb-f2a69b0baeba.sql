...
DECLARE
  ...
  _rpc_cents bigint;
  _progress jsonb;
  _expected_cents bigint := 50000;  -- 500 EUR HTVA
BEGIN
  -- Setup vendeur + customer + margin_rule (window = calendar_year pour aligner hook & RPC)
  INSERT INTO public.vendors (id, name, slug, company_name, is_active, country_code, auth_user_id)
  VALUES (gen_random_uuid(), _marker || '_V', lower(_marker) || '-v', _marker || ' SRL', true, 'BE', gen_random_uuid())
  ...
  -- 1) VALIDE : delivered, dans l'année en cours (500 EUR HTVA)
  ...
  INSERT INTO public.order_lines (order_id, vendor_id, quantity, unit_price_excl_vat, unit_price_incl_vat,
                                  vat_rate, line_total_excl_vat, line_total_incl_vat)
  VALUES (_o_valid, _vendor, 1, 500.00, 605.00, 21.00, 500.00, 605.00);

  -- 2) EXCLU : status=cancelled
  INSERT INTO public.orders (id, order_number, customer_id, status, subtotal_excl_vat, total_incl_vat, created_at)
  VALUES (gen_random_uuid(), _marker || '_can', _customer, 'cancelled', 100.00, 121.00, _year_start + interval '11 days')
  RETURNING id INTO _o_cancelled;
  INSERT INTO public.order_lines (order_id, vendor_id, quantity, unit_price_excl_vat, unit_price_incl_vat,
                                  vat_rate, line_total_excl_vat, line_total_incl_vat)
  VALUES (_o_cancelled, _vendor, 1, 100.00, 121.00, 21.00, 100.00, 121.00);

  -- 3) EXCLU : is_forecast=true
  INSERT INTO public.orders (id, order_number, customer_id, status, subtotal_excl_vat, total_incl_vat,
                             is_forecast, created_at)
  VALUES (gen_random_uuid(), _marker || '_fc', _customer, 'confirmed', 200.00, 242.00,
          true, _year_start + interval '12 days')
  RETURNING id INTO _o_forecast;
  INSERT INTO public.order_lines (order_id, vendor_id, quantity, unit_price_excl_vat, unit_price_incl_vat,
                                  vat_rate, line_total_excl_vat, line_total_incl_vat)
  VALUES (_o_forecast, _vendor, 1, 200.00, 242.00, 21.00, 200.00, 242.00);

  -- 4) EXCLU : is_test=true
  INSERT INTO public.orders (id, order_number, customer_id, status, subtotal_excl_vat, total_incl_vat,
                             is_test, created_at)
  VALUES (gen_random_uuid(), _marker || '_ts', _customer, 'confirmed', 300.00, 363.00,
          true, _year_start + interval '13 days')
  RETURNING id INTO _o_test;
  INSERT INTO public.order_lines (order_id, vendor_id, quantity, unit_price_excl_vat, unit_price_incl_vat,
                                  vat_rate, line_total_excl_vat, line_total_incl_vat)
  VALUES (_o_test, _vendor, 1, 300.00, 363.00, 21.00, 300.00, 363.00);

  -- 5) EXCLU : hidden_from_list=true
  INSERT INTO public.orders (id, order_number, customer_id, status, subtotal_excl_vat, total_incl_vat,
                             hidden_from_list, created_at)
  VALUES (gen_random_uuid(), _marker || '_hd', _customer, 'delivered', 400.00, 484.00,
          true, _year_start + interval '14 days')
  RETURNING id INTO _o_hidden;
  INSERT INTO public.order_lines (order_id, vendor_id, quantity, unit_price_excl_vat, unit_price_incl_vat,
                                  vat_rate, line_total_excl_vat, line_total_incl_vat)
  VALUES (_o_hidden, _vendor, 1, 400.00, 484.00, 21.00, 400.00, 484.00);

  -- 6) EXCLU : deleted_at non nul
  INSERT INTO public.orders (id, order_number, customer_id, status, subtotal_excl_vat, total_incl_vat,
                             deleted_at, created_at)
  VALUES (gen_random_uuid(), _marker || '_dl', _customer, 'delivered', 250.00, 302.50,
          now(), _year_start + interval '15 days')
  RETURNING id INTO _o_deleted;
  INSERT INTO public.order_lines (order_id, vendor_id, quantity, unit_price_excl_vat, unit_price_incl_vat,
                                  vat_rate, line_total_excl_vat, line_total_incl_vat)
  VALUES (_o_deleted, _vendor, 1, 250.00, 302.50, 21.00, 250.00, 302.50);

  -- 7) EXCLU : hors fenêtre (année précédente)
  INSERT INTO public.orders (id, order_number, customer_id, status, subtotal_excl_vat, total_incl_vat, created_at)
  VALUES (gen_random_uuid(), _marker || '_py', _customer, 'delivered', 999.00, 1208.79, _prior_year - interval '30 days')
  RETURNING id INTO _o_prior;
  INSERT INTO public.order_lines (order_id, vendor_id, quantity, unit_price_excl_vat, unit_price_incl_vat,
                                  vat_rate, line_total_excl_vat, line_total_incl_vat)
  VALUES (_o_prior, _vendor, 1, 999.00, 1208.79, 21.00, 999.00, 1208.79);

  -- (a) Réplique SQL EXACTE des filtres de useVendorMonthlyDashboard
  ...
  _rpc_cents := COALESCE((_progress->>'gmv_cents')::bigint, 0);
  ...
  details := 'gmv_cents = ' || _rpc_cents::text || ' | rule window = calendar_year';
  RETURN NEXT;

  -- (c) PARITÉ stricte hook ↔ RPC
  ...
  -- (d) Aucune commande exclue ne fuit
  ...
  DELETE FROM public.order_lines
    WHERE order_id IN (_o_valid, _o_cancelled, _o_forecast, _o_test, _o_hidden, _o_deleted, _o_prior);
  DELETE FROM public.orders
    WHERE id IN (_o_valid, _o_cancelled, _o_forecast, _o_test, _o_hidden, _o_deleted, _o_prior);
...
REVOKE ALL ON FUNCTION public.vendor_gmv_filters_self_test() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.vendor_gmv_filters_self_test() TO service_role;

CREATE OR REPLACE FUNCTION public.vendor_gmv_filters_self_test_admin()
RETURNS TABLE(scenario text, expected bigint, actual bigint, ok boolean, details text)
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path = public AS $$
BEGIN
  ...
  END IF;
  RETURN QUERY SELECT * FROM public.vendor_gmv_filters_self_test();
END;
$$;

REVOKE ALL ON FUNCTION public.vendor_gmv_filters_self_test_admin() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.vendor_gmv_filters_self_test_admin() TO authenticated;
