CREATE OR REPLACE FUNCTION public.admin_list_orders(
  ...
  _buyer_type text DEFAULT 'all'::text,
  _payment_status text DEFAULT 'all'::text,
  _billing_status text DEFAULT 'all'::text,
  _sort_by text DEFAULT 'date'::text,
  _sort_dir text DEFAULT 'desc'::text,
  _billing_updated_from timestamp with time zone DEFAULT NULL::timestamp with time zone,
  _billing_updated_to timestamp with time zone DEFAULT NULL::timestamp with time zone
)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE
      ...
      AND (
        ...
        OR c.customer_type = _buyer_type
      )
      AND (
        COALESCE(_payment_status, 'all') = 'all'
        OR COALESCE(o.payment_status, 'pending') = _payment_status
      )
  ),
  filtered AS (
  ...
  page AS (
    ...
    ORDER BY
      ...
      CASE WHEN _sb = 'payment' AND _sd = 'asc'  THEN COALESCE(o.payment_status, '') END ASC,
      CASE WHEN _sb = 'payment' AND _sd = 'desc' THEN COALESCE(o.payment_status, '') END DESC,
