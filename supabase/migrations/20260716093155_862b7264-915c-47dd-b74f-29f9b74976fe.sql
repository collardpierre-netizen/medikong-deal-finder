...
-- 1) Fix admin_create_manual_order: persist v_line_commission, tag source
...
-- 2) Fix admin_update_manual_order: persist v_line_commission, tag source
...
-- 3) Backfill commission_amount for NULL lines with a rate
UPDATE public.order_lines
SET commission_amount = round(
  CASE
    WHEN commission_basis = 'margin' AND line_cost IS NOT NULL
      THEN GREATEST(line_total_excl_vat - line_cost, 0) * commission_rate / 100.0
    ELSE line_total_excl_vat * commission_rate / 100.0
  END,
  2
)
WHERE commission_amount IS NULL
  AND commission_rate IS NOT NULL
  AND line_total_excl_vat IS NOT NULL;

-- 4) Backfill orders.source = 'manual_admin' pour toutes les commandes admin
