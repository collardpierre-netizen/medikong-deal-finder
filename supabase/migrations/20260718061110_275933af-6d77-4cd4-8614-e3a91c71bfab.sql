    ...
    SELECT o.*,
      ...
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'subtotal_excl_vat', so.subtotal_excl_vat,
  ...
  agg_sub_commission AS (
    SELECT COALESCE(SUM(
      COALESCE(
        ...
        CASE
          WHEN so.commission_rate_override IS NOT NULL AND so.subtotal_excl_vat IS NOT NULL
          THEN so.subtotal_excl_vat * so.commission_rate_override / 100
          ELSE 0
        END
      )
