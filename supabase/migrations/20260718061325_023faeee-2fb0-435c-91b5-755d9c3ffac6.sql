...
BEGIN
    ...
    SELECT o.*,
      ...
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'subtotal_excl_vat', (
            SELECT COALESCE(SUM(ol.line_total_excl_vat), 0)
            FROM public.order_lines ol
            WHERE ol.order_id = so.order_id AND ol.vendor_id = so.vendor_id
          ),
  ...
  agg_sub_commission AS (
    SELECT COALESCE(SUM(
      COALESCE(
        s.commission_amount_override,
        CASE
          WHEN s.commission_rate_override IS NOT NULL AND s.sub_subtotal_excl_vat IS NOT NULL
          THEN s.sub_subtotal_excl_vat * s.commission_rate_override / 100
          ELSE 0
        END
      )
