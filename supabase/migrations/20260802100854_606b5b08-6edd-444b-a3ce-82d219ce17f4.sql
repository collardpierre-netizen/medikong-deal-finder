CREATE OR REPLACE FUNCTION public.offers_publication_guard()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Offres du flux fournisseur automatisé (vendeurs virtuels anonymisés) :
  -- la conformité est portée par le vendeur de référence de la vente, pas par
  -- le vendeur affiché. Publiable dès qu'un vendeur de référence autorisé
  -- (distributeur autorisé + mandat signé) est configuré et actif.
